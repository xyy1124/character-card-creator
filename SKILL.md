...
description: 引导式角色卡创建助手 + JSON 生成器：通过问答引导用户完善角色设定，最终输出 Chara Card V2 兼容 JSON 角色卡文件（RisuAI / SillyTavern / 类脑等平台通用；可选用 LnnLore tracker 状态协议扩展）
---
...
> **开源公共版说明**
>
> 本技能为公共开源版，已去除 NSFW/成人向特化内容与个人定制工作流（特定平台登录还原、私人素材源等），可直接用于任何题材的角色卡制作。
>
...
- 典型用途：命令 AI 在每次回复后输出状态面板（如好感度追踪面板等），或执行固定的结算规则。
- 如果角色卡没有特殊需求，可以留空字符串 ""。
...
每条 3-8 个关键词，建议包括：正式名称、常用简称、别名、角色可能使用的称呼、该条目独有的相关词。
...
3. 每个条目必须包含：条目名称、3-8 个激活关键词、独立完整的正文、与角色的关系、被触发后的互动影响或剧情用途。
...
> **强制要求**：用户名等特定用户称呼**不得写入**写作特化世界书——世界书描述的是角色的行为模式，不是某个特定用户的互动历史。
...
> **🔴 强制入口**：任何新建、还原或修改含状态栏（tracker）角色卡的任务，在生成 JSON 前都必须阅读本节并执行第 6 小节「协议格式硬性要求（v76/v77）」。**禁止**复制旧卡 tracker、**禁止**手写旧式简化 schema、**禁止**仅凭 JSON 可解析或字段存在性判定通过。生成脚本必须在写盘前调用公共验证器 `scripts/verify_tracker_v76_v77.js`（本仓库自带）。

### 1. 状态栏三件套（ST 原生变量 + 正则 + Quick Replies + HTML 面板）
...
- `aliases`（可选）：字段口语别名（"好感"等）——本地解析与裁判判断时与 label/key 同等匹配
...
12. **回复规划**——正文必须重点加强的内容 / 是否必须输出状态面板 / 预计回复风格
...
- **双位置强制**：mes_example 示范之外，在 system_prompt 第一小节写入「## 思维链强制输出（最高优先级）」条款（12 步框架清单 + think 格式要求，放在其他规则之前）——mes_example 示范在新对话中可能不被注入或仅被当作参考，system_prompt 强制段才是保证任何对话都执行的关键。
