...
const POLLUTION_RE = /必须输出状态面板|输出 HTML 状态面板|每次回复末尾(?:强制)?输出[^\n]{0,12}(?:状态面板|状态栏|面板)|输出状态栏|状态面板强制输出|角色卡是否要求状态面板|是否必须输出状态面板|更新面板|回复末尾完整输出状态面板|预测更新后的状态栏|状态栏必须与剧情一致|在 post_history_instructions 中更新面板/;
// 模拟 App 剥离后 PHI 残留的面板样式（剥离后命中即 FAIL）——允许自然叙述的状态叙事约束段（如【状态叙事约束】信任度在温柔戳破时上升），禁止面板值样式（【标签】:数字/占位符/爱心）
const PHI_STRIP_RESIDUE_RE = /状态面板|状态栏|❤~|当前心理状态|【\{|【[^】]{1,12}】[:：]?\s*(?:[0-9]|【|\{)/;
